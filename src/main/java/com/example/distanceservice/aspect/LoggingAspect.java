...
public class LoggingAspect {
    ...
    public void logAfterReturning(JoinPoint joinPoint, Object result) {
        LOGGER.info("Метод " + joinPoint.getSignature().getName() + " вернул: " + result);
    }

    @AfterThrowing(pointcut = "execution(* " + CONTROLLER_PACKAGE + ")", throwing = "exception")
    public void logAfterThrowing(JoinPoint joinPoint, Throwable exception) {
        LOGGER.error("Исключение в методе " + joinPoint.getSignature().getName() + ": " + exception.getMessage(), exception);
    }
}
